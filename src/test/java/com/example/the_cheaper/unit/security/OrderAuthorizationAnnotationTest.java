...
class OrderAuthorizationAnnotationTest {
    ...
    void adminOrderEndpointsUseAdminOrderPermissions() throws Exception {
        assertEquals("hasRole('ADMIN') or hasAuthority('ORDER_READ')", preAuthorize(AdminOrderController.class, "getListOrders",
                com.example.the_cheaper.dto.request.admin.AdminOrderFilterRequest.class));
        assertEquals("hasRole('ADMIN') or hasAuthority('ORDER_READ')", preAuthorize(AdminOrderController.class, "getOrderDetail", Long.class));
        assertEquals("@orderAccess.canUpdate(authentication, #p1)", preAuthorize(AdminOrderController.class, "updateOrderStatus", Long.class,
    ...
    void userOrderEndpointsUseUserOrderPermissions() throws Exception {
        assertEquals("hasRole('ADMIN') or hasAuthority('USER_ORDER_CREATE')", preAuthorize(UserOrderController.class, "createOrder",
                com.example.the_cheaper.entity.AccountEntity.class,
                com.example.the_cheaper.dto.request.user.UserCreateOrderRequest.class));
        assertEquals("hasRole('ADMIN') or hasAuthority('USER_ORDER_READ')", preAuthorize(UserOrderController.class, "getMyOrders",
                com.example.the_cheaper.entity.AccountEntity.class, int.class, int.class));
        assertEquals("hasRole('ADMIN') or hasAuthority('USER_ORDER_READ')", preAuthorize(UserOrderController.class, "getOrderDetail",
                com.example.the_cheaper.entity.AccountEntity.class, Long.class));
        assertEquals("hasRole('ADMIN') or hasAuthority('USER_ORDER_CANCEL')", preAuthorize(UserOrderController.class, "cancelOrder",
                com.example.the_cheaper.entity.AccountEntity.class, Long.class));
    }
